...
class Critic(nn.Module):
    def __init__(self, state_dim, action_dim,hidden_dim=(256,256), drop_rate=0.0):
...
class Agent(object):
    def __init__(self, state_dim, goal_dim, action_dim, max_action, hidden_dim=(256, 256),
        ...
        self.critic = Critic(state_dim + goal_dim, action_dim, hidden_dim, drop_rate).to(device)
    ...
    def train(self, replay_buffer, demos, normalizers=(0, 1, 0, 1), iterations=2):
        for it in range(iterations):
            ...
            with torch.no_grad():
                ...
                target_q = reward + (1 - done) * self.gamma * torch.min(targetQ1, targetQ2)
                demos_targetQ1, demos_targetQ2 = self.critic_target(demos_next_input, demos_next_action)
                demos_target_q = demos_reward + (1 - demos_done) * self.gamma * torch.min(demos_targetQ1, demos_targetQ2)
            ...
            if self.total_it % self.policy_freq == 0:
                ...
                Q_dem_set = self.critic.Q1(input_repeat, action_repeat).reshape(1000, self.batch_size_demo)
                demos_Q_set = self.critic.Q1(input_repeat, policy_actions_repeat).reshape(1000, self.batch_size_demo)
