    ...
    def train(self, replay_buffer, demos, normalizers=(0, 1, 0, 1), iterations=2):
        for it in range(iterations):
            ...
            if self.total_it % self.policy_freq == 0:
                ...
                demos_Q_std1 = demos_Q_std[0]
                demos_Q_std1 = demos_Q_std1.detach().cpu().item()
                Q_dem_std1 = Q_dem_std[0]
                Q_dem_std1 = Q_dem_std1.detach().cpu().item()
                demos_Q1=demos_Q[0]
                demos_Q1=demos_Q1.detach().cpu().item()
                Q_dem1=Q_dem[0]
                Q_dem1=Q_dem1.detach().cpu().item()
                ...
                for param, target_param in zip(self.actor.parameters(), self.actor_target.parameters()):
                    target_param.data.copy_(self.tau * param.data + (1 - self.tau) * target_param.data)
        return num_accept, num_total, demos_Q_std1, Q_dem_std1, demos_Q1, Q_dem1
