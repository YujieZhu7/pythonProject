...
class Agent(object):
    def __init__(self, state_dim, goal_dim, action_dim, max_action, hidden_dim=(256, 256),
        ...
        self.fixedcritic = VectorizedCritic(state_dim + goal_dim, action_dim, ensemble_size, hidden_dim, drop_rate).to(device)
        ...
        for it in range(iterations):
            ...
            if self.total_it % self.policy_freq == 0:
                ...
                if self.method == "First":  # usual Q-Filter method
                    ...
                    Q_dem = self.fixedcritic(demos_input, demos_action)[0]
                if self.method == "Mean":
                    demos_Q_set = self.critic(demos_input, demos_policy_actions)
                    Q_dem_set = self.fixedcritic(demos_input, demos_action)
