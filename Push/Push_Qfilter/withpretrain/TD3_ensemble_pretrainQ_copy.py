...
class Agent(object):
    def __init__(self, env, state_dim, goal_dim, action_dim, max_action, hidden_dim=(256, 256),
    ...
    def pretrainQ(self, replay_buffer, normalizers=(0, 1, 0, 1), iterations=2):
        for it in range(iterations):
            ...
            with torch.no_grad():
                ...
                #     next_action_np=np.array([self.env.action_space.sample() # * self.max_action
                #                              for _ in range(self.batch_size_buffer)])
                #     next_action = torch.Tensor(next_action_np).to(self.device)
                #     next_action = next_action * self.max_action
                # else:
                #     next_action = self.actor_target(next_input)
                #     var = 1
                #     noise_d_torch = torch.randn_like(action) * (self.max_action * var)
                #     next_action = (next_action + noise_d_torch).clamp(-self.max_action, self.max_action)
                #     # noise_d = np.random.normal(0, self.max_action * var, size=self.action_dim)
                #     # next_action = np.clip(action + noise_d, -self.max_action, self.max_action)
                #     noise = (torch.randn_like(action) * self.policy_noise).clamp(-self.noise_clip, self.noise_clip)
                #     next_action = (next_action + noise).clamp(-self.max_action, self.max_action)

                next_action = []
                deterministic_action = self.actor_target(next_input)

                for i in range(self.batch_size_buffer):
                    if random.random() <= 0.5:
                        single_action = torch.Tensor(self.env.action_space.sample()).to(self.device)
                        scaled_action = single_action * self.max_action
                    else:
                        scaled_action = deterministic_action[i]
                        var = 1
                        noise_d_torch = torch.randn_like(scaled_action) * (self.max_action * var)
                        scaled_action = (scaled_action + noise_d_torch).clamp(-self.max_action, self.max_action)

                        noise = (torch.randn_like(scaled_action) * self.policy_noise).clamp(-self.noise_clip, self.noise_clip)
                        scaled_action = (scaled_action + noise).clamp(-self.max_action, self.max_action)
                    next_action.append(scaled_action)
