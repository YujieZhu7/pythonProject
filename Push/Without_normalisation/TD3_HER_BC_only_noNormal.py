...
class Agent(object):
    ...
    def train(self, replay_buffer, demos, iterations=2):
        for it in range(iterations):
            ...
            next_input = np.concatenate([next_state, goal], axis=1)
            ...
            demos_input = np.concatenate([demos_state, demos_goal], axis=1)
            demos_input = torch.Tensor(demos_input).to(self.device)
            demos_next_input = np.concatenate([demos_next_state, demos_goal], axis=1)
