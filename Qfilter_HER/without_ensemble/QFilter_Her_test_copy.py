...
import random
import pickle
import TD3_HER_Qfilter_copy as TD3
...
while steps < max_steps + 1:
    ...
    if len(replay_buffer) > learning_starts:
        ...
        #                                                                   goal_stats[0], np.sqrt(goal_stats[1])),
        #                                               iterations=2, BC_only=True)
        # else:
        #     num_accept, num_total = agent.train_demos(demos, normalizers=(state_stats[0], np.sqrt(state_stats[1]),
        #                                                                   goal_stats[0], np.sqrt(goal_stats[1])),
        #                                               iterations=2, BC_only=False)
...
# np.save(f"Results/{env_name}/QFilter_TD3HER_NonExpert_Method_{method}_EnsSize_{ensemble_size}_%acceptdemos_S{seed}",
#         percent_accept_demos)
